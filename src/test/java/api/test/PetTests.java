...
public class PetTests {
	...
	@BeforeClass
	public static void petdatasetup()
	{
		fake=new Faker();
		petpayload=new Pet();
	...
	public void TestGetPet()
	{
		...
		Assert.assertEquals(this.petpayload.getCategory().getId(),res.jsonPath().getInt("category.id"));
		Assert.assertEquals(this.petpayload.getCategory().getName(),res.jsonPath().getString("category.name"));
		Assert.assertEquals(this.petpayload.getName(),res.jsonPath().getString("name"));
		Assert.assertEquals(this.petpayload.getPhotoUrls()[0],res.jsonPath().getString("photoUrls[0]"));
		Assert.assertEquals(this.petpayload.getTags()[0].getId(),res.jsonPath().getInt("tags[0].id"));
		Assert.assertEquals(this.petpayload.getTags()[0].getName(),res.jsonPath().getString("tags[0].name"));
		Assert.assertTrue(res.getTime()<3000);
		Assert.assertEquals(res.getContentType(),"application/json");  //Method 1 of checking content type
		Assert.assertEquals(res.getHeader("Content-Type"),"application/json");
		Assert.assertNotNull(res.getBody());   //to check if response body is NULL
